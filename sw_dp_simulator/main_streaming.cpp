...
//#include "sketches/dd_iteration.h"
...
int main(int argc, char* argv[])
{
    ...
    while(true) {
        ...
        c = getopt_long(argc, argv, "a:cd:e:f:g:hi:j:k:l:mn:o:p:r:t:uw:z:x:", long_options, &option_index);
		...
		switch(c) {
            // common parameters
            case 'x':
                memcpy(params.inject, optarg, 1000);
                break;
			case 'f':
...
//    print_flowkey_flags(params.flowkey_flags);
    ...
    // TODO: fill bin limit based on params
    //LogCollapsingLowestDenseDDSketch dd_iteration(1024);
    ...
    //else if(strcmp(run, "dd") == 0) {
    //    sketch_iteration_instance = &dd_iteration;
    //}
    else {
    ...
    if(params.is_hardware) {
        ...
        pcap_t* handle = init_pcap_parse_streaming(params.pcap_file_path);
        main_loop_streaming(params, sketch_iteration_instance, handle);
    }
