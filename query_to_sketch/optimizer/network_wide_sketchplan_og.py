...
def optimal_baseline_placement(metrics, unique_sketches, metric_sketch_map, metric_min_sketch_resource_config, profiles, od_pairs, devices, device_rams, od_pair_metric_map, run_strawman=False, time_limit=0):
    ...
    if run_strawman:
        for sketch in list(profiles.keys()):
            for metric in list(profiles[sketch].keys()):
                if metric in metric_sketch_map and metric_sketch_map[metric] == sketch:
                    profiles[sketch][metric] = {metric_min_sketch_resource_config[metric]: 0}
                else:
                    # dummy to ensure num_profiles_per_sketch is correct and consistent across metrics
                    profiles[sketch][metric] = {(0, 0, 0): 0}

        #for metric in metric_sketch_map.keys():
        #    sketch = metric_sketch_map[metric]
        #    profiles[sketch][metric] = {metric_min_sketch_resource_config[metric]: 0}
        #    for other_metrics in profiles[sketch
...
def main(args):
    global OD_PAIRS, CLUSTER_DEVICE_MAP, METRICS, DEVICE_SRAM, DEVICE_RAMS, SKETCHES
    if args.od_pkl:
